...
class Crossword:
    ...
    def __init__(self, **kwargs):
        ...
        logging.debug(
            "Crossword __init__: n={}, m={}, fname={}".format(
                self.rows, self.cols, self.words_file
            )
        )
        # Loading words
        logging.debug(
            "Crossword __init__: Started loading words from {}".format(self.words_file)
        )
        arr = np.genfromtxt(self.words_file, dtype="str", delimiter=" ")
        self.words = arr[np.in1d(arr[:, 3], ["v", "n", "adv", "a"])][:, 2].tolist()
        # Number of words loaded
        logging.debug(
            "Crossword __init__: Number of words loaded: {}".format(len(self.words))
        )
        self.words = list(
            set(x for x in self.words if len(x) <= self.rows and len(x) <= self.cols)
        )
        if self.sort:
            self.words = sorted(self.words, key=len, reverse=self.maximize_len)
        # After filter logging
        logging.debug(
            "Crossword __init__: Number of words after filter: {}, maxlen = {}".format(
                len(self.words), len(max(self.words, key=len))
            )
        )

    def _init_board(self):
        logging.debug(
            "Crossword init_board: initing a list, rows={}, cols={}".format(
                self.rows, self.cols
            )
        )
    ...
    def _erase(self, word):
        ...
        if word.horizontal:
            for i in range(word.col, word.col + word.length):
                if not (
                    (
                        self._check_inside(word.row - 1, i)
                        and self.board[word.row - 1][i][0]
                    )
                    or (
                        self._check_inside(word.row + 1, i)
                        and self.board[word.row + 1][i][0]
            ...
            for i in range(word.row, word.row + word.length):
                if not (
                    (
                        self._check_inside(i, word.col - 1)
                        and self.board[i][word.col - 1][0]
                    )
                    or (
                        self._check_inside(i, word.col + 1)
                        and self.board[i][word.col + 1][0]
    ...
    def _check_word(self, word, start_row, start_col, horizontal, end_row, end_col):
        ...
        # Initial checks
        if (
            not self._check_inside(start_row, start_col)
            or not self._check_inside(end_row, end_col)
            or self._check_exists(start_row, start_col, horizontal)
        ):
            return False

        # Checking if there's another word near the end
        if (
            self._check_inside(end_row + vert_one, end_col + hor_one)
            and self.board[end_row + vert_one][end_col + hor_one][0] is not None
        ):
            return False

        # Checking if there's another word near the start
        if (
            self._check_inside(start_row - vert_one, start_col - hor_one)
            and self.board[start_row - vert_one][start_col - hor_one][0] is not None
        ):
            return False
        # check_neg = ''
        # check_pos = ''
        for i in range(len(word)):
            ...
            if (
                self._check_inside(cur_pos[0] + hor_one, cur_pos[1] + vert_one)
                and self.board[cur_pos[0] + hor_one][cur_pos[1] + vert_one][1]
                not in [self.VERTICAL if horizontal else self.HORIZONTAL, 0]
            ) or (
                self._check_inside(cur_pos[0] - hor_one, cur_pos[1] - vert_one)
                and self.board[cur_pos[0] - hor_one][cur_pos[1] - vert_one][1]
                not in [self.VERTICAL if horizontal else self.HORIZONTAL, 0]
    ...
    def _check_cross(self, word_board, word_string, letter_pos):
        ...
        intersect_pos = [
            index
            for index, letter in enumerate(word_board.word)
            if letter == word_string[letter_pos]
        ]
        for ind in intersect_pos:
            start_pos = (
                word_board.row + (ind if horizontal else -1 * letter_pos),
                word_board.col + (0 if horizontal else ind),
            )
            end_pos = (
                start_pos[0] + (0 if horizontal else (len(word_string) - 1)),
                start_pos[1] + ((len(word_string) - 1) if horizontal else 0),
            )
            if self._check_inside(start_pos[0], start_pos[1]) and self._check_word(
                word_string,
                start_pos[0],
                start_pos[1],
                horizontal,
    ...
    def _save(self, file_path, time, max_time, algo, res, num_words):
        np.save(
            file_path,
            np.array(
                [self.current_words, res, self.setup, time, max_time, algo, num_words]
            ),
        )
...
def results(algo="forward"):
    ...
    for board in settings[0]["boards"][0]:
        c = Crossword(
            ...
            m=board["m"],
            sort=board["sort"],
            maximize_len=board.get("max", False),
            repeat_words=board.get("dupes", False),
        )
