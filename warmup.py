...
class BBoard():
    ...
    def backtracking_rec(self, row, col):
        for letter in self.alphabet:
            if self.check(letter, row, col):
                ...
                if not self.next_pos(row,col) or \
                       self.backtracking_rec(*self.next_pos(row,col)):
                    return True
                else:
                    self.board[row, col] = ''
        return False
    ...
    def backtracking(self):
        for letter in self.alphabet:
            if self.check(letter, 0, 0):
                ...
                if not self.next_pos(0, 0) or \
                       self.backtracking_rec(*self.next_pos(0, 0)):
    ...
    def frw_rec(self, row, col):
        for letter in self.frw_options[row][col]:
            ...
            if not self.next_pos(row,col) or \
                   self.frw_rec(*self.next_pos(row,col)):
                return True
            else:
                self.board[row, col] = ''
        return False
    ...
    def forward_checking(self):
        for letter in self.frw_options[0][0]:
            ...
            if not self.next_pos(0, 0) or \
                   self.frw_rec(*self.next_pos(0, 0)):
                return True, self.board
            else:
                self.board[0, 0] = ''
        return False
