...
def home():
    if request.method == 'POST':
        ...
        Crossword(n=int(request.form.get('rows')),
                  m=int(request.form.get('cols')),
                  repeat_words=bool(int(request.form.get('repeats'))),
                  sort=bool(request.form.get('sort')),
                  maximize_len=bool(int(request.form.get('sort')) - 1)).generate(int(request.form.get('num_words')),
...
def show_crossword(crossword_id):
    ...
    if os.path.isfile(arr_path):
        ...
        return render_template_string('Please wait for a process to finish.')
